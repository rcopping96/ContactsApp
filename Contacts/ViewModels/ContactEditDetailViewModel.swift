...
struct ContactEditDetailViewModel: View {
    ...
    @State private var addDOB: Bool = false
    ...
    var body: some View {
            Form {
                ...
                Section() {
                    if addDOB == true {
                            DatePickerViewModel(placeholder: "Add Birthday", date: $dateOfBirth)
                    } else {
                        Button(action: {
                            self.addDOB.toggle()
                        }, label: {
    ...
    func firebaseAction(newContact: Bool) {
        let contact = Contact(firstName: firstName, lastName: lastName, company: company, dateOfBirth: dateOfBirth, addressLine1: addressLine1, addressLine2: addressLine2, city: city, county: county, postcode: postcode, phoneNumber: phoneNumber, email: email, favorite: favorite, userId: Auth.auth().currentUser?.uid)
